...
\begin{equation}
 \partial_tu=D\nabla^2u+u(1-u)(u-a).
\end{equation}
FitzHugh--Nagumo is the two-field activator--inhibitor reaction system; its
coefficients are those of the released solver configuration. For both problems the
reaction terms change each field's total, so neither conserves a field.
...
\begin{equation}
 ...
 \qquad W'(u)=u^3-u.
\end{equation}
This fourth-order equation conserves mass on a periodic domain. The solver clips its
state to $[-1,1]$, so the distillation target is that clipped map rather than the
unconstrained continuum equation. Bounded PI-NCA conserves mass, not the Cahn--Hilliard
free energy, and no discrete energy law is claimed for it.
...
\begin{align}
 ...
 \partial_tv&=D_v\nabla^2v+uv^2-(F+k)v.
\end{align}
The feed and removal terms change field totals. The original configuration at
$\Delta t=2.0$ exceeds the explicit diffusion limit and diverges; the benchmark uses
$\Delta t=1.0$. Errors remain large for every model.

\paragraph{Navier--Stokes.}
The vorticity formulation reconstructs the flow through the Poisson relation
$\nabla^2\psi=-\omega$, which is the source of global coupling. Viscosity, forcing,
the treatment of the zero Fourier mode and the time discretization are those of the
released solver configuration.
...
The run begins with a correctness gate of 233 tests that must pass before any benchmark
...
The rankings and paired tests quoted in the tables are recomputed from the saved
per-initial-condition errors by \model{pinca\_tests.py}, which is released with the
manuscript, and a bibliography check resolves every cited arXiv identifier against the
arXiv API.
