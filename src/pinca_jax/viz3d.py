...
def render(pde, grid=16, epochs=120, eval_steps=32, every=2, seed=42):
    ...
    model_step = (lambda x: jnp.clip(model.apply(params, x), clip[0], clip[1])) if clip \
        else (lambda x: model.apply(params, x))
...
def main():
    ...
    args = ap.parse_args()
    render(args.pde, grid=args.grid, epochs=args.epochs)
